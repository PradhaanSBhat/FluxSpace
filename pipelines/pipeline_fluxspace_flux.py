...
class FluxPipeline(
    ...
    def get_token_pos(
        self,
        prompt: Union[str, List[str]],
        edit_tokens: Union[str, List[str]] = None,
    ):
        r"""
        ...
        Args:
            ...
            edit_token (`str` or `List[str]`, *optional*):
                edit_token to obtain attention mask
        """
        if edit_tokens is None:
            ...
            for token in edit_tokens:
                ...
                token_pos.extend(token_indices)
            token_pos = list(dict.fromkeys(token_indices))
        return token_pos
    ...
    def check_inputs(
        ...
        ######################################
        if edit_prompt is not None and edit_prompt_embeds is not None:
            raise ValueError(
                f"Cannot forward both `edit_prompt`: {edit_prompt} and `prompt_embeds`: {edit_prompt_embeds}. Please make sure to"
                " only forward one of the two."
            )
        elif edit_prompt is None and edit_prompt_embeds is None:
            raise ValueError(
                "Provide either `edit_prompt` or `edit_prompt_embeds`. Cannot leave both `edit_prompt` and `edit_prompt_embeds` undefined."
            )
        elif edit_prompt is not None and (
            not isinstance(edit_prompt, str) and not isinstance(edit_prompt, list)
        ):
            raise ValueError(
                f"`edit_prompt` has to be of type `str` or `list` but is {type(edit_prompt)}"
            )
        ######################################
        ...
        if prompt_embeds is not None and edit_prompt_embeds is not None:
            if prompt_embeds.shape != edit_prompt_embeds.shape:
                raise ValueError(
                    "`prompt_embeds` and `edit_prompt_embeds` must have the same shape when passed directly, but"
                    f" got: `prompt_embeds` {prompt_embeds.shape} != `negative_prompt_embeds`"
                    f" {edit_prompt_embeds.shape}."
                )
        ...
        ######################################
        if edit_prompt_embeds is not None and edit_pooled_prompt_embeds is None:
            raise ValueError(
                "If `edit_prompt_embeds` are provided, `edit_pooled_prompt_embeds` also have to be passed. Make sure to generate `edit_pooled_prompt_embeds`."
            )
        ######################################
    ...
    # Copied from https://github.com/huggingface/diffusers/blob/425a715e35479338c06b2a68eb3a95790c1db3c5/examples/community/pipeline_flux_rf_inversion.py#L553
    ...
    def __call__(
        ...
        edit_token: Union[str, List[str]] = None,
        ######################################
        ...
        edit_iteration: int = 0,
        fine_guidance_scale: float = 0.0,
        coarse_guidance_scale: float = 0.0,
        thresholding_mask: float = 0.0,
        ######################################
        ...
        pooled_prompt_embeds: Optional[torch.FloatTensor] = None,
        ########################
        edit_pooled_prompt_embeds: Optional[torch.FloatTensor] = None,
        ########################
        ...
        Args:
            ...
            edit_prompt (`str` or `List[str]`, *optional*):
                The prompt or prompts to be used to guide the image edit. If not defined, one has to pass `edit_prompt_embeds`
                instead.
            edit_token (`str` or `List[str]`, *optional*):
                The token used to apply fine fluxspace edit
            ########################
            ...
            edit_iteration (`int`, *optional*, defaults to 0):
                The iteration from which to apply the fine edit as defined in [FluxSpace](https://huggingface.co/papers/2412.09611)
            coarse_guidance_scale (`float`, *optional*, defaults to 0):
                Coarse edit scale as defined in [FluxSpace](https://huggingface.co/papers/2412.09611) in equation 10.
            fine_guidance_scale (`float`, *optional*, defaults to 0):
                Fine edit scale as defined in [FluxSpace](https://huggingface.co/papers/2412.09611) in equation 5.
            thresholding_mask (`float`, *optional*, defaults to 0):
                Thresholding mask as defined in [FluxSpace](https://huggingface.co/papers/2412.09611) in equation 8.
            thresholding_mask
            ########################
            ...
            edit_prompt_embeds (`torch.FloatTensor`, *optional*):
                Pre-generated edit text embeddings. Can be used to easily tweak text inputs, *e.g.* prompt weighting. If not
                provided, text embeddings will be generated from `edit_prompt` input argument.
            ########################
            pooled_prompt_embeds (`torch.FloatTensor`, *optional*):
                Pre-generated pooled text embeddings. Can be used to easily tweak text inputs, *e.g.* prompt weighting.
                If not provided, pooled text embeddings will be generated from `prompt` input argument.
            ########################
            edit_pooled_prompt_embeds (`torch.FloatTensor`, *optional*):
                Pre-generated pooled edit text embeddings. Can be used to easily tweak text inputs, *e.g.* prompt weighting.
                If not provided, pooled edit text embeddings will be generated from `edit_prompt` input argument.
            ########################
        ...
        self.check_inputs(
            ...
            ########################
            edit_pooled_prompt_embeds=edit_pooled_prompt_embeds,
            ########################
        ...
        ########################
        has_edit_prompt = edit_prompt is not None or (
            edit_prompt_embeds is not None and edit_pooled_prompt_embeds is not None
        )
        ...
        if has_edit_prompt:
            (
                ...
                pooled_prompt_embeds=edit_pooled_prompt_embeds,
                device=device,
                num_images_per_prompt=num_images_per_prompt,
                max_sequence_length=max_sequence_length,
                lora_scale=lora_scale,
        ...
        # FluxSpace Coarse Editing Logic
        fluxspace_kwargs = {}
        eps = 1e-08
        ...
        dirn = (
            (edit_pooled_prompt_embeds * pooled_prompt_embeds).sum(dim=-1, keepdim=True)
            / (
                (pooled_prompt_embeds * pooled_prompt_embeds).sum(dim=-1, keepdim=True)
                + eps
            )
        ) * pooled_prompt_embeds  # B, D
        dirn = edit_pooled_prompt_embeds - dirn

        # Equation (10)
        dirn = (
            1 - coarse_guidance_scale
        ) * pooled_prompt_embeds + coarse_guidance_scale * dirn
        ########################

        ########################
        # FluxSpace Fine Editing Logic
        null_prompt = ""
        null_prompt_embeds, _, _ = self.encode_prompt(
            prompt=null_prompt,
            prompt_2=null_prompt,
            prompt_embeds=None,
            pooled_prompt_embeds=None,
            device=device,
            num_images_per_prompt=num_images_per_prompt,
        ...
        fluxspace_kwargs["edit_encoder_hidden_states"] = edit_prompt_embeds
        fluxspace_kwargs["null_encoder_hidden_states"] = null_prompt_embeds
        fluxspace_kwargs["fine_guidance_scale"] = fine_guidance_scale
        if fluxspace_kwargs["fine_guidance_scale"] > 0.0:
            token_pos = self.get_token_pos(prompt=edit_prompt, edit_tokens=edit_token)
            fluxspace_kwargs["token_pos"] = token_pos
            fluxspace_kwargs["thresholding_mask"] = thresholding_mask
        ########################
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                ########################
                if i < edit_iteration:
                    fluxspace_kwargs["fine_guidance_scale"] = 0
                    fluxspace_kwargs["text_modulate_pooled_projections"] = None
                else:
                    fluxspace_kwargs["text_modulate_pooled_projections"] = dirn
                    fluxspace_kwargs["fine_guidance_scale"] = fine_guidance_scale
                ########################
                ...
                noise_pred = self.transformer(
                    ...
                    ########################
                    fluxspace_kwargs=fluxspace_kwargs,
                    ########################
                    return_dict=False,
                )[0]
    ...
    def invert(
        ...
        with self.progress_bar(total=N - 1) as progress_bar:
            for i in range(N - 1):
                ...
                timestep = t_i.detach().clone().repeat(batch_size)
